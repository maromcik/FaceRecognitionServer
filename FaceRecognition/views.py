...
def facerecognition():
    ...
    while True:
        ...
        access = rec_threads.access_pool.apply_async(rec_threads.rec.access, args=(labels, frame, rec_threads.arduino_lock))
...
def index(request):
    ...
    try:
        ...
        running = False
    return HttpResponse(render(request, 'LiveView/LiveView.html', {'running': running, 'subscription': "test"}))
...
def start(request):
    ...
    return HttpResponse(render(request, 'LiveView/LiveView.html',
                               {'message': message, 'running': running, 'subscription': "test",
                                'status': status}))
